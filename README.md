...
[![Code Quality](https://github.com/shaneholloman/packer-vsphere/actions/workflows/code-quality.yml/badge.svg)](https://github.com/shaneholloman/packer-vsphere/actions/workflows/code-quality.yml)
...
> **Note**
>
> - The Microsoft Windows 11 machine image uses a virtual trusted platform module (vTPM). Refer to the VMware vSphere [product documentation][vsphere-tpm] for requirements and pre-requisites.
>
...
1. Create service account in vSphere SSO if it does not exist: In the left pane, select **Single Sign On** > **Users and Groups** and click on **Users**, from the dropdown select the domain in which you want to create the user (_e.g.,_ mgmt.nz) and click **ADD**, fill all the username (_e.g.,_ svc-packer-vsphere) and all required details, then click **ADD** to create the user.
1. In the left pane, select **Access control** > **Global permissions** and click the **Add permissions** icon.
1. In the **Add permissions** dialog box, enter the service account (_e.g.,_ <svc-packer-vsphere@mgmt.nz>), select the custom role (_e.g.,_ Packer to vSphere Integration Role) and the **Propagate to children** checkbox, and click OK.
...
```sh
./config.sh config/production
./build.sh config/production
```

**Cello:** customer

```sh
./config.sh config/customer
./build.sh config/customer
```
...
```sh
shadminphoton> sudo systemctl start docker
shadminphoton> sudo docker run -it --rm alpine:latest
mkpasswd -m sha512
Password: ***************
[password hash]
shadminphoton> sudo systemctl stop docker
```

**Example**: mkpasswd using Docker on macOS:

```sh
shadmin> docker run -it --rm alpine:latest
mkpasswd -m sha512
Password: ***************
[password hash]
```
...
```sh
shadminubuntu> mkpasswd -m sha-512
Password: ***************
[password hash]
```
...
```sh
shadmin> ssh-keygen -t ecdsa -b 521 -C "code@mgmt.nz"
Generating public/private ecdsa key pair.
Enter file in which to save the key (/Users/shadmin/.ssh/id_ecdsa):
Enter passphrase (empty for no passphrase): **************
Enter same passphrase again: **************
Your identification has been saved in /Users/shadmin/.ssh/id_ecdsa.
Your public key has been saved in /Users/shadmin/.ssh/id_ecdsa.pub.
```
...
```hcl
communicator_proxy_host     = "proxy.mgmt.nz"
communicator_proxy_port     = 8080
communicator_proxy_username = "shadmin"
...
```hcl
vsphere_endpoint             = "sfo-w01-vc01.sfo.mgmt.nz"
vsphere_username             = "svc-packer-vsphere@mgmt.nz"
...
```sh
shadmin> export HCP_CLIENT_ID=<client_id>
shadmin> export HCP_CLIENT_SECRET=<client_secret>
```
...
```sh
shadmin> packer init builds/windows/server/2022/.
```

Build a specific machine image:

```sh
shadmin> packer build -force \
...
```sh
shadmin> gomplate -c build.yaml -f build.tmpl -o build.sh
```
...
- Read [VM Tools for Powershell][vm-tools]
...
[vm-tools]: https://github.com/getvpro/Build-Packer/blob/master/Scripts/Install-VMTools.ps1
